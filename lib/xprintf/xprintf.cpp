...
/* Code borrowed from PS2DEV Open Source project, which was borrowed from mysql's xprintf.c, by Richard Hipp */
/* The xprintf.c file on which this one is based is in public domain. */
    ...
    */
    switch( xtype ){
      ...
      case RADIX:
	    ...
	    signed long t = xva_arg(ap,signed long);
	    longvalue = t;
	}else if(( flag_long )&&( !infop->flag_signed )){
	    unsigned long t = xva_arg(ap,unsigned long);
	    longvalue = t;
	}else if(( !flag_long )&&( infop->flag_signed )){
	    signed int t = xva_arg(ap,signed int) & ((unsigned long) 0xffffffff);
	    longvalue = t;
	}else{
	    unsigned int t = xva_arg(ap,unsigned int) & ((unsigned long) 0xffffffff);
	    longvalue = t;
	}
#ifdef COMPATIBILITY
	...
	if(infop->fmttype == 'p')
      ...
      case CHARLIT:
      case CHAR:
        c = buf[0] = (xtype==CHAR ? xva_arg(ap,int) : *++fmt);
        if( precision>=0 ){
      ...
      case STRING:
      case MEM_STRING:
        zMem = bufpt = xva_arg(ap,char*);
        if( bufpt==0 ) bufpt = (char*)"(null)";
        length = strlen(bufpt);
